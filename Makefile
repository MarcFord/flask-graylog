...
# Test the package
test:
	uv run pytest --cov=src/flask_graylog --cov-report=term-missing -v
...
lint:
	...
	uv tool run mypy src/flask_graylog/ --python-version 3.9 --ignore-missing-imports
	uv tool run bandit -r src/flask_graylog/
...
check-version:
	...
	@python -c "import src.flask_graylog; \
		package_version = src.flask_graylog.__version__; \
...
ci-test:
	uv run pytest --cov=src/flask_graylog --cov-report=xml --cov-report=term-missing -v
