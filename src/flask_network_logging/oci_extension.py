        ...
        if self.logging_client and log_id:
            try:
                oci_handler = OCILoggingHandler(
                    ...
                    log_id=log_id,
                    source=self.config.get('OCI_SOURCE', 'flask-app')
                )
...
class OCILoggingHandler(logging.Handler):
    ...
    def _create_log_entry(self, record: logging.LogRecord, message: str) -> Dict[str, Any]:
        ...
        for attr in ['request_id', 'user_id', 'username', 'ip_address', 'user_agent',
                     'endpoint', 'method', 'url', 'args', 'form', 'json']:
        ...
        if hasattr(record, '__dict__'):
            for key, value in record.__dict__.items():
                if (key not in ['name', 'msg', 'args', 'levelname', 'levelno',
                               ...
                               'threadName', 'processName', 'process', 'message'] and
