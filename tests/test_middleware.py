...
class TestMiddleware:
    ...
    def test_setup_middleware(self):
        ...
        with patch.object(app, 'before_request') as mock_before, \
             patch.object(app, 'after_request') as mock_after:
...
class TestMiddlewareIntegration:
    ...
    @pytest.mark.parametrize("extension_class,config", [
        ("AWSLogExtension", {
        ...
        ("IBMLogExtension", {
            'IBM_LOGDNA_INGESTION_KEY': 'test-key',
            'IBM_LOGDNA_HOSTNAME': 'test-host',
            'IBM_ENVIRONMENT': 'ibm'
        }),
        ("OCILogExtension", {
            'OCI_LOG_ID': 'test-log-id',
            'OCI_ENVIRONMENT': 'oci'
        })
    ])
